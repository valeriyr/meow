...
A `BlockHeader` contains seven fields:
...
| `reward_root` | `Option<Digest>` | Digest of the reward transaction; `None` when no gas was collected |
...
- **Block hash** — Blake2b-256 of the BCS-serialized full header (all seven fields). Used as the block's identity and stored in the next block's `parent_hash`.
- **Mining hash** — Blake2b-256 of a reduced projection containing only `height`, `parent_hash`, `transactions_root`, `timestamp`, and `nonce` — **excluding `state_root` and `reward_root`**. This is the value checked against the PoW target and used as the VM randomness seed.

`state_root` and `reward_root` are excluded from the mining hash because both are only known after re-executing every transaction — which requires the nonce to already be fixed. `transactions_root` is included so the miner commits to the exact transaction set before grinding begins and cannot swap transactions after finding a favorable nonce.
