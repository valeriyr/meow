...
//
// Digest signing tests.
//

#[test]
fn sign_produces_deterministic_digest() {
    let d1 = Digest::sign(b"hello").unwrap();
    let d2 = Digest::sign(b"hello").unwrap();
    assert_eq!(d1, d2);
}

#[test]
fn sign_different_inputs_produce_different_digests() {
    let d1 = Digest::sign(b"hello").unwrap();
    let d2 = Digest::sign(b"world").unwrap();
    assert_ne!(d1, d2);
}

#[test]
fn sign_not_zero() {
    let digest = Digest::sign(b"hello").unwrap();
    assert_eq!(
        digest.to_string(),
