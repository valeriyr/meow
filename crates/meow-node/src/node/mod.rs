...
pub struct Node {
    rpc_addr: SocketAddr,
    gossip_network_config: GossipNetworkConfig,
...
impl Node {
    ...
    pub fn empty(
        ...
        Self {
            ...
            miner: Arc::new(Mutex::new(Miner::empty(difficulty))),
        }
    }
    ...
    pub fn with_genesis(
        ...
        Self {
            rpc_addr,
            gossip_network_config,
            miner: Arc::new(Mutex::new(Miner::with_genesis(genesis, difficulty))),
        }
    }
    ...
    async fn run_internal(
        ...
        tracing::info!(
            ...
            gossip_listen_addr = %self.gossip_network_config.listen_address,
            bootstrap_peers = self.gossip_network_config.bootstrap_peers.len(),
        ...
        let tcp_listener = tokio::net::TcpListener::bind(self.rpc_addr)
            .await
        ...
        let gossip_service = GossipService::new(self.miner.clone(), transactions_rx, blocks_rx);
        ...
        let gossip_future = gossip_service.run(self.gossip_network_config, shutdown_rx.clone());
        ...
        macro_rules! await_service_shutdown {
            ($service_name:literal, $future:ident, $err_variant:path) => {
                match (&mut $future).await {
                    ...
                    Err(e) => {
                        tracing::error!(
                            "{} service terminated with error during shutdown: {e}",
                            $service_name
                        );
        ...
        tokio::select! {
            ...
            rpc_result = &mut rpc_future => {
                match rpc_result {
                    Ok(()) => {
                        tracing::warn!("rpc service terminated gracefully");
                    }
                    Err(e) => {
                        tracing::error!("rpc service terminated with error: {e}");
            ...
            miner_result = &mut miner_future => {
                match miner_result {
                    Ok(()) => {
                        tracing::warn!("miner service terminated gracefully");
                    }
                    Err(e) => {
                        tracing::error!("miner service terminated with error: {e}");
            ...
            gossip_result = &mut gossip_future => {
                match gossip_result {
                    Ok(()) => {
                        tracing::warn!("gossip service terminated gracefully");
                    }
                    Err(e) => {
                        tracing::error!("gossip service terminated with error: {e}");
