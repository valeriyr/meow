use meow_types::{identifier::Identifier, system_framework::meow_coin::MeowCoin};
use meow_vm_adapter::{
    builder,
...
fn burn_destroys_coin() {
    ...
    for balance in [0u64, 50, 1000] {
        let result = run("burn", vec![make_coin(id, balance)])
...
fn split_with_sufficient_balance() {
    let id = [0xCCu8; 32];
    let result =
        run("split", vec![make_coin(id, 100), Value::U64(40)]).expect("split must succeed");
...
fn split_with_insufficient_balance() {
    let id = [0xDDu8; 32];
    let err = run("split", vec![make_coin(id, 10), Value::U64(20)])
...
fn mint_with_zero_balance_succeeds() {
    let owner = [0x01u8; 32];
    let result = run("mint", vec![Value::U64(0), Value::Address(owner.into())])
...
fn split_with_exact_balance_zeroes_from() {
    let id = [0xFFu8; 32];
    let result = run("split", vec![make_coin(id, 50), Value::U64(50)]).expect("split must succeed");
