...
impl CallArg {
    ...
    pub fn into_input(self, client: &NodeClient) -> anyhow::Result<Input> {
        Ok(match self {
            CallArg::Bool(b) => Input::Raw(bcs::to_bytes(&b)?),
            CallArg::U64(n) => Input::Raw(bcs::to_bytes(&n)?),
            CallArg::Address(addr) => Input::Raw(bcs::to_bytes(&addr)?),
            CallArg::Str(str) => Input::Raw(bcs::to_bytes(&str)?),
            CallArg::Object(addr) => Input::Object(get_object(client, &addr)?.object_ref()),
        })
    }
    ...
    pub fn into_value(self, client: &NodeClient) -> anyhow::Result<Value> {
        Ok(match self {
            ...
            CallArg::Object(addr) => {
                let obj = get_object(client, &addr)?;
...
fn get_object(client: &NodeClient, addr: &Address) -> anyhow::Result<Object> {
    client
        .get_object(addr)?
